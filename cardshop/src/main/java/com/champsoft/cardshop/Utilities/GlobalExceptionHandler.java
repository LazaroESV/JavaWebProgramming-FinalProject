...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleCardNotFound(CardNotFoundException ex) {
    ...
    public ResponseEntity<ProblemDetail> handleCollectorNotFound(CollectorNotFoundException ex) {
        ProblemDetail pd = ProblemDetail.forStatus(HttpStatus.NOT_FOUND);
        pd.setTitle("Collector Not Found");
        pd.setDetail(ex.getMessage());
        pd.setProperty("errors", List.of(Map.of("field", "id", "message", ex.getLocalizedMessage())));
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(pd);
    }
    ...
    public ResponseEntity<ProblemDetail> handleInvalidCardData(InvalidCardDataException ex) {
        ProblemDetail pd = ProblemDetail.forStatus(HttpStatus.BAD_REQUEST);
        pd.setTitle("Invalid Card Data");
        pd.setDetail("Invalid Card Data");
    ...
    // 409 : Conflicts
    @ExceptionHandler(CollectorOwnsCardsException.class)
    public ResponseEntity<ProblemDetail> handleCollectorOwnsCards(CollectorOwnsCardsException ex) {
        ProblemDetail pd = ProblemDetail.forStatus(HttpStatus.CONFLICT);
        pd.setTitle("Collector Owns Cards");
        pd.setDetail(ex.getMessage());
        pd.setProperty("errors", List.of(Map.of("field", "collector", "message", ex.getMessage())));
